...
async fn create_and_get_calendar_event() {
    ...
    let calendar_event_input = CalendarEvent {
        ...
        // start_time: DateTime::parse_from_str("2023-01-01 00:00:00", "%Y-%m-%d %H:%M:%S").unwrap_or().into(),
        // end_time: DateTime::parse_from_str("2023-01-01 01:00:00", "%Y-%m-%d %H:%M:%S").unwrap().into(),
